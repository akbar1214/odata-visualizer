...
server.registerTool(
  ...
  {
    description:
      'Load OData V4 metadata from a file path or URL. Parses the CSDL XML and stores the schema for use by other tools. Call this first before using other tools.',
    inputSchema: {
      source: z
        .string()
        .describe(
          'File path (e.g., "./metadata.xml") or URL (e.g., "https://host/Windchill/servlet/odata/ProdMgmt/$metadata")',
        ),
      type: z
        .enum(['file', 'url'])
...
server.registerTool(
  'list_functions',
  {
    ...
    inputSchema: {
      bound: z.boolean().optional().describe('Filter to bound (true) or unbound (false) functions'),
      ...paginationSchema,
    },
  },
...
server.registerTool(
  'build_query',
  {
    ...
    inputSchema: {
      ...
      filters: z.array(filterSchema).optional(),
      filterLogic: z.enum(['and', 'or']).optional().describe('How to join filters (default "and")'),
      select: z.array(z.string()).optional(),
      expand: z.array(expandNodeSchema).optional(),
